...
class MakePageViewsRollupsUnique < ActiveRecord::Migration[4.2]
  ...
  def up
    scope = PageViewsRollup.select(%i[course_id date category]).group(:course_id,
                                                                      :date,
    ...
    rename_index :page_views_rollups,
                 "index_page_views_rollups_on_course_id_and_date_and_category",
